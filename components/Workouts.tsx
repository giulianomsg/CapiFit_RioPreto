...
const PlanCard: React.FC<{ name: string; description: string }> = ({ name, description }) => (
  <div className="bg-light-card dark:bg-dark-card p-6 rounded-xl shadow-md cursor-pointer transform hover:scale-105 transition-transform duration-200">
    <div className="flex items-center justify-between mb-4">
      ...
      <div className="text-primary">{ICONS.workouts}</div>
    </div>
...
export const Workouts: React.FC<WorkoutsProps> = ({ workoutPlans, onAddPlan }) => {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold">Modelos de Treino</h2>
      ...
      {workoutPlans.length > 0 ? (
        ...
        <div className="text-center py-12 bg-light-card dark:bg-dark-card rounded-xl">
            <h3 className="text-xl font-semibold">Nenhum modelo de treino encontrado</h3>
            <p className="text-gray-500 dark:text-gray-400 mt-2">Crie modelos de treino para reutilizar com seus alunos.</p>
        </div>
      )}

      {isModalOpen && <PlanCreator type="Workout" onClose={() => setModalOpen(false)} onSave={handleSavePlan} />}
    </div>
  );
};
